...
def build_run_regressor(model, name, ax=None, feature_columns=None, color=None):
    ...
    # # multiply features  by 2^16 to avoid floating point issues
    # train[feature_columns] = train[feature_columns] * 2**16
    # test[feature_columns] = test[feature_columns] * 2**16
    ...
    # save to a file for comparison, test_pred is a list
    test_pred_model_filename = os.path.join(out_dir, f'{name}_test_model_pred.csv')
    numpy.savetxt(test_pred_model_filename, test_pred, delimiter=',', fmt='%f')
    # print MSE/MAE of test_pred using sklearn.metrics.accuracy_score
    # print(f'{name} python model accuracy: {sklearn.metrics.r2_score(test[target_column], test_pred)}')
    # MSE
    print(f'{name} python model MSE: {sklearn.metrics.mean_squared_error(test[target_column], test_pred)}')
    # MAE
    print(f'{name} python model MAE: {sklearn.metrics.mean_absolute_error(test[target_column], test_pred)}')
    ...
    # print(f'{name} cpp model (emlearn) accuracy: {sklearn.metrics.r2_score(test[target_column], test_pred)}')
    # MSE
    print(f'{name} cpp model (emlearn) MSE: {sklearn.metrics.mean_squared_error(test[target_column], test_pred)}')
    # MAE
    print(f'{name} cpp model (emlearn) MAE: {sklearn.metrics.mean_absolute_error(test[target_column], test_pred)}')




    # save the test data in csv file
    test_filename = os.path.join(out_dir, f'{name}_test.csv')
    test.to_csv(test_filename, index=False)

    # save predictions to a file for comparison, test_pred is a list
    test_pred_cpp_filename = os.path.join(out_dir, f'{name}_test_cpp_pred.csv')
    numpy.savetxt(test_pred_cpp_filename, test_pred, delimiter=',', fmt='%f')
